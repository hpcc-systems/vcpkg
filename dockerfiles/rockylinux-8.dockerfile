...
RUN set -euxo pipefail && \
    curl -L "https://github.com/westes/flex/releases/download/v${FLEX_VERSION}/flex-${FLEX_VERSION}.tar.gz" -o /tmp/flex.tar.gz && \
    tar -xzf /tmp/flex.tar.gz -C /tmp && \
    cd "/tmp/flex-${FLEX_VERSION}" && \
    ./configure --prefix=/usr/local && \
    make -j"$(nproc)" && \
    make install && \
    curl -L "https://ftp.gnu.org/gnu/bison/bison-${BISON_VERSION}.tar.gz" -o /tmp/bison.tar.gz && \
